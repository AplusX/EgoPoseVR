"""
Utility functions for 3D rotation transformations.
Supports conversions between axis-angle, 6D, quaternion, and rotation matrix representations.
"""
...
def aa2quat(pose_aa):
    """
    :param pose_aa: Nx3
    :return: pose_quat: Nx4
    """
    pose_sixd = aa2sixd(pose_aa)
    pose_quat = sixd2quat(pose_sixd)
    return pose_quat
...
def sixd2quat(pose_6d):
    ...
    pose_mat_34 = torch.cat((pose_mat, torch.zeros(pose_mat.size(0), pose_mat.size(1), 1, device="cuda:0")), dim=-1)
...
def quat2sixd(pose_quat):
    """
    :param pose_quat: Nx4
    :return: pose_6d: Nx6
    """
    pose_aa = tgm.quaternion_to_angle_axis(pose_quat)
